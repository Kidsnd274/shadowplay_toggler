...
<img src="assets/icon/app_icon.png" alt="ShadowPlay Toggler icon" width="128" height="128" />

# ShadowPlay Toggler

**Stop NVIDIA's overlay from treating your desktop apps like games.**

A clean, safe Windows utility for managing per-application NVIDIA capture
...
[![GPU: NVIDIA](https://img.shields.io/badge/GPU-NVIDIA-76B900?logo=nvidia&logoColor=white)](#requirements)
...
## Why ShadowPlay Toggler?

NVIDIA's overlay and capture stack (NVIDIA Share / Instant Replay /
ShadowPlay / Overlay) sometimes mistakes normal desktop applications —
**SignalRGB, OBS, Paint.NET, Discord**, etc. — for games. When that happens
it can hijack hotkeys, start recording the wrong window, or cause random
stutters.

The official workaround is to edit an undocumented NVIDIA Driver Settings
(DRS) flag (`0x809D5F60`) with tools like NVIDIA Profile Inspector. That's
error-prone, easy to forget about, and gets wiped by driver reinstalls.

**ShadowPlay Toggler makes this a one-click job** and keeps track of
everything you've changed so you never lose your setup.

## Features

- **One-click exclusions** — drop in an `.exe` (or drag-and-drop) and it's
  instantly excluded from NVIDIA capture/overlay.
- **Managed rules list** — see every app you've excluded, with live status
  dots showing whether the driver still agrees with you.
- **Detect existing rules** — discover overrides already on your driver
  (left there by Profile Inspector, old installs, etc.) and adopt them
  into the app with one click.
- **NVIDIA defaults view** — browse the predefined NVIDIA profiles that
  already carry the capture-exclusion flag, for reference.
- **Safe rollback** — removing a rule restores the driver default instead
  of guessing an "off" value, so you can't accidentally break anything.
- **Startup reconciliation** — every time you launch, the app checks the
  driver against your saved rules and tells you if anything drifted or got
  wiped by a driver reinstall.
- **Backup & restore** — export your entire DRS configuration to a file
  and import it back later (uses NVIDIA's own save/load APIs).
- **Batch actions** — enable or disable multiple rules at once.
- **No telemetry, no network calls** — everything is local.

## Requirements

- **Windows 10 or newer** (NVAPI is Windows-only).
- An **NVIDIA GPU** with a recent NVIDIA driver installed.

That's it — no runtime to install, no admin rights needed.

## Installation

Grab the latest release from the
[**Releases page**](../../releases), unzip it anywhere, and run
`shadowplay_toggler.exe`.

> Prefer to build from source? See [Building from source](#building-from-source) below.

## Usage

1. **Launch the app.** It'll scan your driver on startup and show anything
   it already knows about.
2. **Add an app to exclude:** click *Add Program*, pick an `.exe` (or drag
   it onto the window). The exclusion is applied to the driver
   immediately.
3. **Adopt existing rules:** if you've used Profile Inspector before,
   switch to the *Detected Existing Rules* tab and click *Adopt* on
   anything you want the app to manage going forward.
4. **Remove an exclusion:** select the row and click *Remove*. The
   underlying DRS setting is cleared so the driver falls back to its
   default — it's a clean uninstall of the rule, not an "off" toggle.
5. **Back up your rules:** use *Backup → Export* before driver updates,
   and *Backup → Import* afterwards if NVIDIA wipes your DRS.

If a driver update does reset things, ShadowPlay Toggler will notice on
next launch and offer to re-apply your managed rules in one click.

## How it compares

| | ShadowPlay Toggler | NVIDIA Profile Inspector | GeForce Experience |
|---|:---:|:---:|:---:|
| Per-app capture exclusion | Yes | Yes (manual) | No |
| Tracks which rules are yours | Yes | No | n/a |
| Detects driver resets / drift | Yes | No | n/a |
| One-click adopt existing rules | Yes | No | n/a |
| Safe rollback (restore default) | Yes | Partial | n/a |
| DRS backup / restore | Yes | Yes | No |

## FAQ

**Is this safe? Will it break my driver?**
It only touches a single, well-known DRS setting per profile, and removing
a rule uses NVAPI's *restore default* path rather than writing a guessed
...
No. Everything is local: a small SQLite file in your app data folder and
direct NVAPI calls on your machine.

## Screenshots

> Coming soon — drop PNGs into `docs/screenshots/` and reference them here.

## Reporting bugs & requesting features

Please open an issue on this repository. Include:

- Your Windows version and NVIDIA driver version.
- What you were doing and what happened.
- Any error message shown by the app (the snackbar / error dialog text).

For bugs in **NVAPI itself**, use the
...
Everything below is aimed at people hacking on ShadowPlay Toggler itself.
If you just want to *use* the app, you can stop reading here.
...
The app is a Flutter Windows desktop UI talking over `dart:ffi` to a thin
C++ bridge DLL that wraps NVAPI.
...
joins this against `managedRulesProvider` to render per-row status dots
and, when `hasAnyIssue` is true, surfaces the
...
- DRS profiles can be edited by other tools, wiped by driver reinstalls,
  or repaired by NVIDIA. We must notice and explain these cases instead
  of silently re-writing the user's settings.
...
- A C++17-capable MSVC toolchain — `flutter run -d windows` will use
  whatever Visual Studio Build Tools you have configured for Flutter
  desktop.
...
If you already cloned without `--recursive`, just run the second command
on its own.
...
- **`shadowplay_bridge.dll` not found at runtime** — usually means CMake
  didn't pick up the new bridge sources. Run
...
6. **Persistence** — if you need to read or write local state, go through
   ...
   widgets.
7. **Tests** — add unit tests under `test/` for new services and any
   tricky widget logic. Mock the bridge through `NvapiService`.
...
- All NVAPI calls happen on the main thread inside the DLL. DRS scans run
  ...
  NVAPI.
- JSON returned from the bridge is documented inline above each function
  in `bridge.h`. Update the comment when you change the shape.
...
NVIDIA publishes new SDK drops periodically (the upstream repo mirrors
what ships on the developer portal). To pull a newer version into this
repo:
...
If you want a *specific* tagged release instead of the tip of `main`,
list tags first:
...
- **Do not edit files inside `native/nvapi_sdk/` in place.** They are
  owned by the upstream submodule and any local edits will be wiped on
...
- The capture-exclusion setting we rely on (`0x809D5F60`,
  ...
  community-identified, so always re-test the exclusion round-trip after
  a bump.
...
For project bugs, comments, and feature requests, open an issue in this
repository. Bug reports against NVAPI itself should go to the
[NVAPI Developer Forum](https://forums.developer.nvidia.com/c/developer-tools/other-tools/nvapi/).
